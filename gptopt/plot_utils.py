...
def get_alpha_from_lr(lr, min_alpha=0.5, max_alpha=1.0, lr_range=None):
    ...
    if lr_range and lr_range[0] == lr_range[1]:  # Single learning rate case
        return max_alpha
    return min_alpha + (max_alpha - min_alpha) * (lr - lr_range[0]) / (
        lr_range[1] - lr_range[0]
    )
...
def get_lr_and_name(output):
    ...
    if alg_name == "muon":
        ...
        translate = {
            ...
            "machpolar172": "MachPolar172",
            "machpolar23": "MachPolar23",
        }
    ...
    for output in outputs:
        ...
        if name not in plotted_methods:
            if lr_ranges[name][0] == lr_ranges[name][1]:  # Single learning rate
                ...
                label = (
                    f"{name} lr in [{lr_ranges[name][0]:.4f}, {lr_ranges[name][1]:.4f}]"
                )
...
def plot_step_size_and_lr(ax, outputs, colormap, linestylemap, lr_ranges, alpha_func):
    ...
    for output in outputs:
        if (
            "step_size_list" not in output["logs"]
            or "learning_rates" not in output["logs"]
        ):
            continue
        ...
        if name not in plotted_methods:
            if lr_ranges[name][0] == lr_ranges[name][1]:
                ...
                label = (
                    f"{name} lr in [{lr_ranges[name][0]:.1e}, {lr_ranges[name][1]:.1e}]"
                )

        ax.plot(
            range(len(output["logs"]["step_size_list"])),
        ...
        ax.plot(
            range(len(output["logs"]["learning_rates"])),
