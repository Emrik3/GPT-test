...
captured_grads = defaultdict(list)
...
TARGET_PARAM = "module._orig_mod.transformer.h.3.attn.c_attn.weight"
MAX_CAPTURE_STEPS = 3
        ...
        for batch in val_dataloader:
            with autocast_ctxt:
                val_loss += torch.tensor(
                    model(batch[0], batch[1], return_logits=False)[1], device=device
                )
            counter += 1
            if (val_accum_steps != 0) and (counter >= val_accum_steps):
                break
...
def train(
    ...
    if training_params["autocast"]:
        autocast_ctxt = torch.autocast(
            device_type=device, dtype=typedict[training_params["mixed_precision"]]
        )
    ...
    max_grad_norm = (
        training_params["gradnorm"]
        if training_params["gradnorm"] != 0.0
        else float("inf")
    )

    load_ckpt_step = logging_params["load_ckpt_step"]
    if load_ckpt_step != 0:
        model, optimizer, train_dataloader, scheduler = load_checkpoint(
            ckpt_dir, load_ckpt_step, model, optimizer, train_dataloader, scheduler=None
        )
    if ckpt_dir == "":
    ...
    for epoch in range(training_params["num_epochs"]):
        ...
        for batch in train_dataloader:
            ...
            if step % grad_accum_steps != 0:
                ...
                optimizer_step_count = step // grad_accum_steps
                if optimizer_step_count <= MAX_CAPTURE_STEPS:
                    for name, param in model.named_parameters():
                        if name == TARGET_PARAM and param.grad is not None:
                            captured_grads[TARGET_PARAM].append(
                                param.grad.detach().cpu().clone()
                            )

                    if optimizer_step_count == MAX_CAPTURE_STEPS:
                        grads = captured_grads.get(TARGET_PARAM, [])
                        if len(grads) == 0:
                            print(
                                f"WARNING: No gradients captured for '{TARGET_PARAM}'. Check the param name."
                            )
                            # Print all available param names to help debug
                            for name, param in model.named_parameters():
                                if param.grad is not None:
                                    print(f"  Available: {name}")
                        else:
                            torch.save(torch.stack(grads), "h3_c_attn_grads.pt")
                            print(
                                f"Saved {len(grads)} gradient(s), shape={grads[0].shape}"
                            )
                if world_size > 1:
                ...
                if master_process and wandb_run is not None:
                    ...
                    if hasattr(optimizer, "step_size_list"):
                        wandb_log_dict["train/step_size_list"] = (
                            optimizer.step_size_list
                        )
                    for param_group_ix, param_group in enumerate(
                ...
                if (step % logging_params["log_step"] == 0) & master_process:
                    ...
                    print(
                        f"Time taken : {step_time * 1000:0.1f}ms | Tokens/s : {tps / 1000:0.1f}k | Loss : {loss_accum.item():0.3f}"
                    )

                if step % logging_params["val_step"] == 0:
                    val_loss = eval_validation_loss(
                        model, val_dataloader, val_accum_steps, autocast_ctxt
                    )
                    if master_process and wandb_run is not None:
        ...
        print(
            f"In rank: {rank}, time taken for epoch {epoch + 1} : ",
