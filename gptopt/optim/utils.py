...
def get_optimizer(opt_config: dict, lr=1e-3) -> Tuple[torch.optim.Optimizer, dict]:
    ...
    if name == "sgd":
        ...
        hyperp = {"lr": lr, "weight_decay": opt_config.get("weight_decay", 0)}
...
def get_scheduler(
    config: dict, opt: torch.optim.Optimizer, total_iterations=None
    ...
    if name == "constant":
        ...
        scheduler = get_cosine_schedule_with_warmup(
            opt, num_warmup_steps=num_warmup_steps, num_training_steps=total_iterations
        )
        ...
        def get_lr(step):
            if step < num_warmup_steps:
                ...
                return max(
                    0.1,
                    1.0
                    - (step - num_warmup_steps) / (total_iterations - num_warmup_steps),
                )
