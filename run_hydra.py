...
from gptopt.optim.utils import get_optimizer_factory, get_scheduler
from gptopt.train_distributed import train
from gptopt.utils import get_worker_info, hash_config, set_seed
...
def main(config: DictConfig):
    ...
    ckpt_dir = (
        ...
        if config["logging_params"].get("save_checkpoint", False)
        else ""
    )
    if master_process:
    ...
    scheduler = get_scheduler(
        config["lr_schedule"], optimizer, total_iterations=total_iterations
    )
